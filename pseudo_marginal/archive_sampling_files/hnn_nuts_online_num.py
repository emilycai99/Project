...
from pseudo_marginal.archive_sampling_files.hnn_nuts_online import stop_criterion
...
def build_tree_tf(theta, rho, u, p, log_slice_var, v, j, epsilon, joint0, call_lf, hnn_model, func, args):
    ...
    if j == 0:
        ...
        # one leapfrog step
        # the main modification to use numerical gradients only
        if args.grad_mass_flag:
            hnn_ivp1 = integrator_mass(coords=y1, func=args.grad_func, derivs_func=numerical_grad_debug, h=v*epsilon, steps=1, 
                                target_dim=args.target_dim, aux_dim=args.aux_dim, args=args)
        elif args.grad_flag:
            ...
            hnn_ivp1 = integrator(coords=y1, func=func, derivs_func=numerical_grad, h=v*epsilon, steps=1, 
                                target_dim=args.target_dim, aux_dim=args.aux_dim)
        # new theta, rho, u, p
...
def sample(args):
    ...
    result_path = '{}/results/{}_T{}_n{}_p{}_N{}_ns{}_ls{}_ss{}_lr{}_nhs{}_{}_e{}_mass{}_grad{}_num'.format(args.save_dir, args.dist_name,
                                                                           ...
                                                                           args.nn_model_name, args.epsilon, args.grad_mass_flag,
                                                                           args.grad_flag)
    ...
    # ham = Hamiltonian_func_debug2(args)
    # func = ham.get_func()

    # add a new gradient calculation
    if args.grad_mass_flag:
        cal_grad = calculate_grad_mass(args)
    ...
    for m in range(1, M + Madapt, 1):
        ...
        thetaplus = tf.identity(thetaminus)
        rhoplus = tf.identity(rhominus)
        uplus = tf.identity(uminus)
        pplus = tf.identity(pminus)
        ## initial height
        j = 0
        ...
        while s == 1:
            ...
            if (sprime == 1) and (tf.random.uniform(shape=[]) < _tmp):
                samples[m] = tf.identity(thetaprime)
                rho_sto = tf.identity(rhoprime)
                u_sto = tf.identity(uprime)
                p_sto = tf.identity(pprime)
