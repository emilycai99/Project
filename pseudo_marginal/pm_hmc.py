...
from pseudo_marginal.strang_integrator_hnn import StrangIntegrator


__all__ = [
    'PseudoMarginalHamiltonianMonteCarlo',
    'UncalibratedPseudoMarginalHamiltonianMonteCarlo',
    'make_simple_step_size_update_policy',
]


class UncalibratedPseudoMarginalHamiltonianMonteCarloKernelResults(
    mcmc_util.PrettyNamedTupleMixin,
    collections.namedtuple(
        'UncalibratedPseudoMarginalHamiltonianMonteCarloKernelResults',
        [
...
class PseudoMarginalHamiltonianMonteCarlo(kernel_base.TransitionKernel):
  ...
  def __init__(self,
    ...
    self._impl = metropolis_hastings.MetropolisHastings(
        inner_kernel=UncalibratedPseudoMarginalHamiltonianMonteCarlo(
  ...
  @property
  def pm_args(self):
    return self._impl.inner_kernel.pm_args
  
  @property
  def target_momentum_precision_mat(self):
    return self._impl.inner_kernel.target_momentum_precision_mat
  
  @property
  def target_momentum_cov_mat(self):
    return self._impl.inner_kernel.target_momentum_cov_mat
  
  @property
  def multivariate_normal_dist(self):
    return self._impl.inner_kernel.multivariate_normal_dist
...
class UncalibratedPseudoMarginalHamiltonianMonteCarlo(kernel_base.TransitionKernel):
  ...
  or `MetropolisHastings(UncalibratedPseudoMarginalHamiltonianMonteCarlo(...))`.

  For more details on `UncalibratedPseudoMarginalHamiltonianMonteCarlo`, see
  ...
  def __init__(self,
    ...
    if isinstance(pm_args.rho_var, float):
        rho_cov_mat = tf.eye(pm_args.target_dim, dtype=tf.float32) * pm_args.rho_var
        multivariate_normal_dist = None
    else:
        rho_cov_mat = tf.linalg.diag(tf.constant(pm_args.rho_var, dtype=tf.float32))
        assert len(pm_args.rho_var) == pm_args.target_dim, 'if rho_var is a list, it must have length {}'.format(pm_args.target_dim)
        multivariate_normal_dist = tfd.MultivariateNormalDiag(scale_diag=pm_args.rho_var)
  ...
  @property
  def pm_args(self):
    return self._parameters['pm_args']
  ...
  @mcmc_util.set_doc(PseudoMarginalHamiltonianMonteCarlo.one_step.__doc__)
  def one_step(self, current_state, previous_kernel_results, seed=None):
    with tf.name_scope(mcmc_util.make_name(self.name, 'hmc', 'one_step')):
      ...
      for part_seed, x in zip(seeds, current_state_parts):
        if isinstance(self.pm_args.rho_var, float):
          u = samplers.normal(
              shape=[self.pm_args.target_dim],
              stddev=1.0 if self.pm_args.rho_var == 1.0 else math.sqrt(self.pm_args.rho_var),
              dtype=self._momentum_dtype or dtype_util.base_dtype(x.dtype),
              seed=part_seed)
        else:
          u = tf.cast(self.multivariate_normal_dist.sample(seed=part_seed), self._momentum_dtype or dtype_util.base_dtype(x.dtype))
      
        current_momentum_parts.append(
          tf.concat(
            [u,
            samplers.normal(
              shape=[self.pm_args.aux_dim],
              dtype=self._momentum_dtype or dtype_util.base_dtype(x.dtype),
              seed=part_seed
            )
          ], axis=-1))
      ...
      integrator = StrangIntegrator(
        ...
        target_dim=self.pm_args.target_dim,
        aux_dim=self.pm_args.aux_dim,
      ...
      new_kernel_results = previous_kernel_results._replace(
          log_acceptance_correction=_compute_log_acceptance_correction(
              ...
              precision_mat=self.target_momentum_precision_mat,
              target_dim=self.pm_args.target_dim,
              aux_dim=self.pm_args.aux_dim),
  ...
  @mcmc_util.set_doc(PseudoMarginalHamiltonianMonteCarlo.bootstrap_results.__doc__)
  def bootstrap_results(self, init_state):
    with tf.name_scope(
      ...
      result = UncalibratedPseudoMarginalHamiltonianMonteCarloKernelResults(
...
def _compute_log_acceptance_correction(current_momentums,
  ...
  with tf.name_scope(name or 'compute_log_acceptance_correction'):
    ...
    current_momentums_rho, current_momentums_p = split_rho_p(current_momentums)
    proposed_momentums_rho, proposed_momentums_p = split_rho_p(proposed_momentums)

    current_kinetic_rho = tf.add_n([
        compute_quadratic_form(v, precision_mat, axes) for v, axes
        in zip(current_momentums_rho, shard_axis_names)])
    proposed_kinetic_rho = tf.add_n([
        compute_quadratic_form(v, precision_mat, axes) for v, axes
        in zip(proposed_momentums_rho, shard_axis_names)])
    
    current_kinetic_p = tf.add_n([
        compute_quadratic_form(v, None, axes) for v, axes
        in zip(current_momentums_p, shard_axis_names)])
    proposed_kinetic_p = tf.add_n([
        compute_quadratic_form(v, None, axes) for v, axes
        in zip(proposed_momentums_p, shard_axis_names)])

    return 0.5 * mcmc_util.safe_sum([current_kinetic_rho, current_kinetic_p, -proposed_kinetic_rho, -proposed_kinetic_p])
