...
class PseudoMarginal_NoUTurnSampler(kernel.TransitionKernel):
  ...
  def __init__(self,
    ...
    """
    with tf.name_scope(name or 'PseudoMarginal_NoUTurnSampler') as name:
      ...
      self._pm_nuts_args = pm_nuts_args
      
      # prepare for the M^{-1} case
      if isinstance(pm_nuts_args.rho_var, float):
          rho_cov_mat = tf.eye(pm_nuts_args.target_dim, dtype=tf.float32) * pm_nuts_args.rho_var
          multivariate_normal_dist = None
      else:
          rho_cov_mat = tf.linalg.diag(tf.constant(pm_nuts_args.rho_var, dtype=tf.float32))
          assert len(pm_nuts_args.rho_var) == pm_nuts_args.target_dim, 'if rho_var is a list, it must have length {}'.format(pm_nuts_args.target_dim)
          multivariate_normal_dist = tfd.MultivariateNormalDiag(scale_diag=pm_nuts_args.rho_var)
      ...
      # add the manual gradient calculation
      cal_grad = calculate_grad(pm_nuts_args)
      self._grad_func = cal_grad.grad_total
  ...
  @property
  def pm_nuts_args(self):
    return self._pm_nuts_args
  ...
  def bootstrap_results(self, init_state):
    ...
    with tf.name_scope(self.name + '.bootstrap_results'):
      ...
      if self.pm_nuts_args.grad_flag:
        [
            _,
            _,
            current_target_log_prob,
        ] = strang_impl_hnn.process_args_grad(self.target_log_prob_fn, dummy_momentum, init_state, 
                                              grad_func=self.grad_func, target_dim=self.pm_nuts_args.target_dim,  
                                              aux_dim=self.pm_nuts_args.aux_dim)
      else:
        [
            _,
            _,
            current_target_log_prob,
        ] = strang_impl_hnn.process_args(self.target_log_prob_fn, dummy_momentum, init_state)
      ...
      return NUTSKernelResults(
          ...
          energy=compute_hamiltonian(
              ...
              precision_mat=None if self.pm_nuts_args.rho_var == 1.0 else self.target_momentum_precision_mat,
              target_dim=self.pm_nuts_args.target_dim,
              aux_dim=self.pm_nuts_args.aux_dim),
  ...
  def _start_trajectory_batched(self, state, target_log_prob, seed):
    ...
    with tf.name_scope('start_trajectory_batched'):
      ...
      if isinstance(self.pm_nuts_args.rho_var, float):
        momentum = [
          tf.concat([
            samplers.normal(
              shape=[self.pm_nuts_args.target_dim],
              stddev=1.0 if self.pm_nuts_args.rho_var == 1.0 else math.sqrt(self.pm_nuts_args.rho_var),
              dtype=x.dtype,
              seed=momentum_seeds[i]
            ),
            samplers.normal(
              shape=[self.pm_nuts_args.aux_dim],
              dtype=x.dtype,
              seed=momentum_seeds[i]
            )
  ...
  def _loop_tree_doubling(self, step_size, momentum_state_memory,
    ...
    with tf.name_scope('loop_tree_doubling'):
      ...
      if self.pm_nuts_args.grad_flag:
        integrator = strang_impl_hnn.StrangIntegrator_grad(
            ...
            pm_nuts_args=self.pm_nuts_args)
      else:
        integrator = strang_impl_hnn.StrangIntegrator(
            self.target_log_prob_fn,
            step_sizes=[
                # an additional step to generate direction
                tf.where(d, ss, -ss)
