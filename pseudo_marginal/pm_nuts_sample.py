...
from pseudo_marginal.pm_nuts_hnn import PseudoMarginal_NoUTurnSampler_HNN
...
def sample(args):
    ...
    def print_iter(states, previous_kernel_results):
        global iter_count
        if iter_count % args.print_every == 0:
            print('Iter {}'.format(iter_count))
        iter_count += 1
    ...
    if not args.num_flag:
        states, total_num_grad_steps = tfp.mcmc.sample_chain(
            ...
            kernel=PseudoMarginal_NoUTurnSampler_HNN(
