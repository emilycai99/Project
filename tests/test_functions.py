...
from pseudo_marginal.functions import Hamiltonian_func, Hamiltonian_func_debug
...
def test_gauss_mix_func(mocker, mock_from_pickle):
    '''
    unit test for gauss_mix_func
        - by comparing it with handwritten and for-loop version
    '''
    args = mock_args()
    ...
    rho = tf.random.normal(shape=[args.target_dim], stddev=10.0, dtype=tf.float32)
...
def test_gauss_mix_func_debug(mocker, mock_from_pickle):
    '''
    unit test for gauss_mix_func
        - by comparing it with handwritten and for-loop version
    '''
