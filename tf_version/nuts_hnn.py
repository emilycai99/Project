...
import tf_version.leapfrog_integrator_hnn as leapfrog_impl_hnn
...
class NoUTurnSampler_HNN(kernel.TransitionKernel):
  ...
  def __init__(self,
    ...
    """
    with tf.name_scope(name or 'NoUTurnSampler_HNN') as name:
  ...
  def _load_hnn_model(self, baseline=False):
    ...
    if os.path.exists(os.path.dirname(path)):
      ...
      print('The checkpoint does not exist at {}. We just use a random initialization!'.format(path))
  ...
  def bootstrap_results(self, init_state):
    ...
    with tf.name_scope(self.name + '.bootstrap_results'):
      ...
      ] = leapfrog_impl_hnn.process_args(self.target_log_prob_fn, dummy_momentum,
                                     init_state, hnn_model=self.hnn_model, input_dim=self.hnn_model_args.input_dim)
  ...
  def _loop_tree_doubling(self, step_size, momentum_state_memory,
    ...
    with tf.name_scope('loop_tree_doubling'):
      ...
      integrator_num = leapfrog_impl.SimpleLeapfrogIntegrator(
      ...
      # hnn version
      integrator_hnn = leapfrog_impl_hnn.SimpleLeapfrogIntegrator_HNN(
          self.target_log_prob_fn,
          step_sizes=[
              # an additional step to generate direction
              tf.where(d, ss, -ss)
          ...
          input_dim=self.hnn_model_args.input_dim
          ) 
  ...
  def _loop_build_sub_tree(self,
    ...
    with tf.name_scope('loop_build_sub_tree'):
      ...
      if tf.reduce_any(num_grad_flag):
        ...
        next_target_grad_parts = [tf.where(tf.expand_dims(num_grad_flag, axis=-1), target_grad_num, target_grad)
                                  for target_grad_num, target_grad in zip(next_target_grad_parts_num, next_target_grad_parts)]
...
def compute_hamiltonian(target_log_prob, momentum_parts,
  ...
  # Hamiltonian already happens in a -joint way
